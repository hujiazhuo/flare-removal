...
# Adapted from Wave-Mamba (ACMMM2024) for low-light enhancement
# Modified: predicting flare component instead of direct restoration
# ------------------------------------------------------------------------
...
class UNet(nn.Module):
    """
    Wave-Mamba UNet for Flare Removal.

    Key modification: Instead of direct restoration (out + img),
    we predict the flare component and compute: clean = img - flare_pred

    Output: (clean_image, flare_predicted) - dual output for training
    """
    ...
    def __init__(self, in_chn=3, wf=48, n_l_blocks=[1,1,2], n_h_blocks=[1,1,1], ffn_scale=2):
        ...
        # Predict flare component instead of direct output
        self.last = nn.Conv2d(wf, in_chn, kernel_size=3, stride=1, padding=1, bias=True)

    def forward(self, x):
        img = x  # original input with flare
        ...
        ##### Flare Prediction #####
        # Predict the flare component
        flare_pred = self.last(x_l)

        # Clean image = Input - Flare (subtractive residual)
        clean_image = torch.clamp(img - flare_pred, 0, 1)

        # Return (clean_image, flare_pred) for loss computation
        return clean_image, flare_pred


@ARCH_REGISTRY.register()
class WaveMambaFlare(nn.Module):
    """
    Wave-Mamba for Flare Removal.

    Predicts flare component and computes clean image via subtraction.
    Output: (clean_image, flare_predicted)
    """
    ...
    def encode_and_decode(self, input, current_iter=None):
        clean, flare = self.restoration_network(input)
        return clean, flare
    ...
    def test(self, input):
        ...
        clean, flare = self.encode_and_decode(input)
    ...
    def forward(self, input):
        ...
        For training: use both outputs to compute losses
        For inference: only clean_image is needed
        """
        clean, flare = self.encode_and_decode(input)
        return clean, flare
...
#    print(model)
    ...
    print(f"clean shape: {clean.shape}, flare shape: {flare.shape}")
    print(running_time)
