...
def _to_numpy_mask(mask, input_order='HWC'):
    if mask is None:
        return None
...
def calculate_masked_psnr(img1,
                          img2,
                          crop_border,
                          input_order='HWC',
                          test_y_channel=False,
                          mask=None):
    """PSNR on a masked region.

    If mask is None or empty, fall back to full-image PSNR.
    """
    ...
    img1 = _to_numpy_image(img1, input_order=input_order)
    img2 = _to_numpy_image(img2, input_order=input_order)
    mask = _to_numpy_mask(mask, input_order=input_order)
    ...
    if mask is None or np.sum(mask) <= 0:
        return calculate_psnr(img1, img2, crop_border=0, input_order='HWC', test_y_channel=test_y_channel)
    ...
    mse = np.sum(diff * mask) / np.sum(mask)
    if mse == 0:
        return float('inf')
    max_value = 1. if img1.max() <= 1 else 255.
    return 20. * np.log10(max_value / np.sqrt(mse))
...
def calculate_g_psnr(img1,
                     img2,
                     crop_border,
                     input_order='HWC',
                     test_y_channel=False,
                     mask=None):
    return calculate_masked_psnr(
        img1=img1,
        img2=img2,
...
def calculate_s_psnr(img1,
                     img2,
                     crop_border,
                     input_order='HWC',
                     test_y_channel=False,
                     mask=None):
    # S-PSNR uses inverse mask (1 - mask) for scene/non-flare region
    if mask is not None:
        # Ensure mask is normalized to [0, 1] before inverting
        # Mask may be uint8 with max value not reaching 255 (e.g., 225)
